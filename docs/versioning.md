...
- Tags keep the `v*` shape, so the deploy workflow (which triggers on `v*` tags) keeps working unchanged across the switch.
...
- A `v*` tag is the unit of release. Pushing one triggers the gated deploy (`.github/workflows/deploy.yml`): the test gate runs on the tagged commit, and only a green commit is published.
...
- Plain pushes to `main` never deploy; they only run CI.
